...
zz <- tt |> 
  ...
  correlation()
  plot(summary(zz),show_data = "points", x.text.angle = 60)
```
On ne retrouve qu'une forte corrélation entre `EN1` & `EN2` d'une part ou `douleur` & `type`. Ces deux corrélations sont attendues & donc peu gênantes. 
...
On peut considérer que les patients non algiques (évaluation initiale = 0) ne nécessitent pas d'analgésie & que donc , dans ce cas, le critère principal est rempli même en l'absence de traitement antalgique. 

Le critère principal est alors : 

`évaluation initiale ET réévaluation ET (antalgie OU EN1 = 0)`.
...
tbal <- NULL
zz <- tt |> 
  mutate(analg = (analgesie=="oui") | (en1 == 0)) |> 
...
La faible différence est due au peu d'évaluations & surtout de réévaluations chez les patients ayant une douleur nulle. On peut évaluer ce fait pour la réévaluation ou l'analgésie (uniquement pour les 128 patients ayant eu une évaluation initiale) :
...
zz |> 
  barouiph(reevaluation, en1_f, titre = "", stitre = "p < 0,001")
```
...
Pour le critère principal les comparaisons des deux groupes ont été réalisées par un test de Wilcoxon.
